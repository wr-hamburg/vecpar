...
void parallel_map(vecpar::config c, size_t size, Algorithm algorithm,
  ...
  vecpar::cuda::kernel<<<c.gridSize, c.blockSize, c.memorySize>>>(
      size,
...
void parallel_reduce(vecpar::config c, size_t size, Algorithm algorithm,
  ...
  *lock = 0;
  ...
  vecpar::cuda::rkernel<<<c.gridSize, c.blockSize, c.memorySize>>>(
  ...
  vecpar::cuda::rkernel<<<c.gridSize, c.blockSize, c.memorySize>>>(
      ...
      [=] __device__(int *lock, int *idx) mutable {
        ...
        if (tid == 0) {
          ...
          for (size_t i = 0; i < static_cast<size_t>(c.blockSize) &&
