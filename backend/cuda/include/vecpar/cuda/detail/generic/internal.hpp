...
namespace vecpar::cuda_raw {
    ...
    template<typename Algorithm,
       ...
       cuda_data<T> parallel_map(Algorithm algorithm,
        ...
        DEBUG_ACTION(printf("[MAP] nBlocks:%d, nThreads:%d, memorySize:%zu\n",
                            config.m_gridSize, config.m_blockSize, config.m_memorySize);)

        vecpar::cuda::kernel<<<config.m_gridSize, config.m_blockSize, config.m_memorySize>>>(input.size,
    ...
    template<typename Algorithm,
            ...
            typename... Arguments,
            typename std::enable_if<!std::is_same<T, R>::value, void>::type* = nullptr>
    cuda_data<R> parallel_map(Algorithm algorithm,
                              vecpar::config config,
                              cuda_data<T> input,
                              Arguments... args) {
    ...
    void parallel_reduce(Algorithm algorithm,
                         vecpar::config c,
                         cuda_data<R> result,
    ...
    template<typename Algorithm, typename R>
    void parallel_filter(Algorithm algorithm,
                         vecpar::config c,
                         int *idx,
                         cuda_data<R> d_result,
                         cuda_data<R> partial_result) {
