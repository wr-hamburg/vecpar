...
class chain<vecmem::host_memory_resource, vecmem::vector<R>, vecmem::vector<T>,
            OtherInput...> {
...
public:
  ...
  vecmem::vector<R> execute(vecmem::vector<T> &coll, OtherInput... rest) {
    ...
    CHECK_ERROR(cudaMemcpy(result, d_result.ptr, d_result.size * sizeof(R),
...
private:
  ...
  auto wrapper_first(Algorithm &algorithm) {
    return [&](vecmem::vector<input_t> &coll, OtherInput... otherInput) {
      ...
      CHECK_ERROR(cudaMemcpy(d_data, coll.data(), size * sizeof(T),
      ...
      if constexpr (std::is_base_of<
        ...
        return vecpar::cuda_raw::copy_intermediate_result<Algorithm, result_t>(
            coll, vecpar::cuda_raw::parallel_algorithm(algorithm, m_config,
                                                       input, otherInput...));
      } else if constexpr (std::is_base_of<
                               vecpar::detail::parallel_map<result_t, input_t,
                                                            OtherInput...>,
                               Algorithm>::value) {
...
template <typename R, typename T, typename... OtherInput>
class chain<vecmem::host_memory_resource, R, vecmem::vector<T>, OtherInput...> {
  ...
  auto wrapper_first(Algorithm &algorithm) {
    return [&](vecmem::vector<input_t> &coll, OtherInput... otherInput) {
      ...
      if constexpr (std::is_base_of<
        ...
        return vecpar::cuda_raw::parallel_algorithm<Algorithm, input_t>(
