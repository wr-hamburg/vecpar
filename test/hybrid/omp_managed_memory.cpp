...
namespace {
    ...
    TEST_P(CpuManagedMemoryTest, Parallel_MapOnly) {
        ...
        vecpar::omp::parallel_map(vec->size(),
    ...
    TEST_P(CpuManagedMemoryTest, Parallel_MapReduce_Separately) {
        ...
        vecpar::omp::parallel_reduce(vec->size(), result,
                               [&] TARGET(double* r, double tmp) mutable {
                                   alg.reduce(r, tmp);
                               }, par_result);
    ...
    INSTANTIATE_TEST_SUITE_P(Trivial_ManagedMemory, CpuManagedMemoryTest, testing::ValuesIn(N));
}
